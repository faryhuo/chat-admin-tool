...
const TokenDetails: React.FC<Props> = ({handleCancel}) => {
  ...
  const { getAccessToken,addToken } = useModel('token');
  ...
  const setAccessToken=()=>{
    getAccessToken(username,pwd).then((response)=>{
      //const data
    })
  }
     ...
     <div>
      ...
      >
        ...
        <Form.Item label="Type" name="type"  rules={[{ required: true }]}>
          <Radio.Group options={typeOptions}  optionType="button"   onChange={(e)=>{setType(e.target.value)}}/>
        </Form.Item>
        {type=="token" && <>
